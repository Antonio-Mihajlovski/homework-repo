...
const renderHomePage = dummyCurrent => {
  ...
  <div class="current">
  <p>${dummyCurrent.current.dt}</p>
  <p>${dummyCurrent.current.temp}</p>
  <p>${dummyCurrent.current.feels_like}</p>
  <p>${dummyCurrent.current.wind_speed}</p>
  <img src="http://openweathermap.org/img/wn/${dummyCurrent.current.weather[0].icon}@2x.png"/>
  <p>${dummyCurrent.current.weather[0].icon}</p>
  <p>${dummyCurrent.current.weather[0].description}</p>
  <p>${dummyCurrent.current.humidity}</p>
</div>`;
};
const renderDaily = data => {
    ...
    mainEl.innerHTML +=
    `<div class="daily">
<h1>Date: ${new Date(el.dt * 1000).toLocaleDateString("fr")}</h1>
